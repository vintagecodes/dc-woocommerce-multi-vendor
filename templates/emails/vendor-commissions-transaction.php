...
?><p><?php printf(esc_html__( 'Hello,<br>You have successfully completed a withdrawal of $%s on %s through PayPal. The order details are as follows:', 'dc-woocommerce-multi-vendor' ),  $amount,  get_post_meta($transaction_id, 'paid_date', true)); ?></p>
<?php } else if($transaction_mode == 'direct_bank') { 
?><p><?php printf(esc_html__( 'Hello,<br>This is to notify you that your withdrawal request for $%s on %s has been successfully processed. The order details are as follows:  ', 'dc-woocommerce-multi-vendor' ),  $amount,  get_post_meta($transaction_id, 'paid_date', true)) ?></p>	
<?php }?>
