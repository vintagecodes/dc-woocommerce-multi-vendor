	...
	<tbody>
		...
		if( apply_filters('wcmp_vendor_can_overwrite_customer_support', true) ) {
			foreach ( $vendor_array as $vendor_id => $products) { 
				...
				<?php if($cc == 0) { ?>
					<tr>
					<th style="padding:10px 10px; background:none; border-right: 1px solid #e4e4e4; border-bottom: 1px solid #e4e4e4; width:100%;" align="left" valign="top"><?php echo esc_html_e('Vendor Details', 'dc-woocommerce-multi-vendor'); ?></th>
			...
			?>
			<tr>
					<th style="padding:10px 10px; background:none; border-right: 1px solid #e4e4e4; border-bottom: 1px solid #e4e4e4; width:100%;" align="left" valign="top"><?php echo esc_html_e('Customer Support Details & Product Return Address', 'dc-woocommerce-multi-vendor'); ?></th>
