        ...
        if ( states[ country ] ) {
            if ( $.isEmptyObject( states[ country ] ) ) {
                ...
                countryBox.parent().find('.wcmp_state_to_select').each(function() {
                    ...
                    if ( $statebox.is( 'select' ) ) {
                        $statebox.html( '<option value="">' + wc_country_select_params.i18n_select_state_text + '</option><optgroup label="-------------------------------------"><option value="everywhere">'+ wcmp_vendor_shipping_script_data.everywhere_else_option +'</option></optgroup><optgroup label="-------------------------------------">' + options + '</optgroup>' );
                    }
    ...
    function addMultiInputProperty(multi_input_holder) {
        ...
        if ( multi_input_holder.children('.multi_input_block').length == multi_input_limit )  multi_input_holder.find('.add_multi_input_block').hide();
          ...
          multi_input_holder.children('.multi_input_block').each(function() {
            ...
            $(this).children('.add_multi_input_block').attr( 'title', wcmp_vendor_shipping_script_data.wcmp_multiblick_addnew_help );
            $(this).children('.remove_multi_input_block').attr( 'title', wcmp_vendor_shipping_script_data.wcmp_multiblick_remove_help );
        });
        ...
        multi_input_holder.children('.multi_input_block').children('.add_multi_input_block').off('click').on('click', function() {
          ...
          multi_input_blockEle.children('.multi_input_holder').each(function() {
            ...
            multi_input_blockEle.children('.remove_multi_input_block').off('click').on('click', function() {
                var rconfirm = confirm(wcmp_vendor_shipping_script_data.multiblock_delete_confirm);
                if (rconfirm) {
